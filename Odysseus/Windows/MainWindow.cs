...
public sealed record MainWindowDeps(
    ...
    GatherListRunner GatherLists,
    GatherableIndex Gatherables,
    IOwnGatherer Gatherer,
    Func<uint, int> ItemCount);
...
public sealed class MainWindow : OdysseusWindow
{
    ...
    // ── gather lists: several named lists, each item with a bag-count target ──

    private void DrawGatherLists()
    {
        var lists = Cfg.GatherLists;
        OdysseusTheme.SectionHeader($"GATHER LISTS ({lists.Count})", OdysseusTheme.WakeDim);

        if (lists.Count == 0)
        {
            ImGui.TextColored(OdysseusTheme.TextDisabled, "No lists yet.");
            ImGui.SameLine();
            if (ImGui.SmallButton("+ New list")) { lists.Add(new GatherList()); _d.SaveConfig(); }
            return;
        }
        ...
        ImGui.SameLine();
        if (ImGui.SmallButton("+ New")) { lists.Add(new GatherList()); _gatherListIndex = lists.Count - 1; _d.SaveConfig(); return; }
        ImGui.SameLine();
        if (ImGui.SmallButton("Delete")) { lists.RemoveAt(_gatherListIndex); _d.SaveConfig(); return; }

        var name = list.Name;
        ImGui.SetNextItemWidth(150f);
        if (ImGui.InputText("##glname", ref name, 48)) list.Name = name;
        if (ImGui.IsItemDeactivatedAfterEdit()) _d.SaveConfig();
        ImGui.SameLine();
        var enabled = list.Enabled;
        if (ImGui.Checkbox("Enabled##gl", ref enabled)) { list.Enabled = enabled; _d.SaveConfig(); }
        ImGui.SameLine();
        var prune = list.RemoveCompleted;
        if (ImGui.Checkbox("Remove completed##gl", ref prune)) { list.RemoveCompleted = prune; _d.SaveConfig(); }
        if (ImGui.IsItemHovered())
        ...
        if (ImGui.BeginTable("##gatheritems", 4, ImGuiTableFlags.SizingStretchProp))
        {
            ...
            for (var i = 0; i < list.Items.Count; i++)
            {
                ...
                ImGui.SameLine(0f, 6f);
                ImGui.TextColored(done ? OdysseusTheme.TextDisabled : OdysseusTheme.TextPrimary, _d.Gatherables.NameOf(item.ItemId));
                ...
                ImGui.TableNextColumn();
                ImGui.TextColored(OdysseusTheme.TextDisabled, _d.Gatherer.Where(item.ItemId));
        ...
        {
            foreach (var (id, itemName) in _d.Gatherables.Search(_gatherFilter, 8))
            {
                ...
                ImGui.SameLine();
                ImGui.TextColored(OdysseusTheme.TextDisabled, _d.Gatherer.Where(id));
            }
        }
        OdysseusTheme.EndCard();
        ...
        {
            var canRun = Cfg.Enabled && Cfg.OwnGathering && !Running && lists.Any(l => l.Enabled && l.Items.Count > 0);
            ...
            if (ImGui.IsItemHovered() && !canRun)
                ImGui.SetTooltip(!Cfg.OwnGathering ? "Own gathering is off in Settings." : Running ? "A run is active." : "Nothing on an enabled list.");
